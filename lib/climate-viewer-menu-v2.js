    ...
    if (layerEnabled[layerId] === undefined) {
        ...
        if (l.T === ("wms")) {
           ...
           loadOsmLayer(layerId, geoDataSrc);
            ...
            loadGeoJson(layerId, geoDataSrc, markerLabel, markerScale, markerImg, opacity);
        } else if (l.T === ('kml')) {
    ...
    _.each(layers, function (l) {
        ...
        if (!l.T) {
            ...
            if (present) {
                ...
                var details = $('<div class="ui card ' + layerId + '-details layer-details" />');
                ...
                content = $('<div>').data('l', l).attr('id', l.I).addClass('lbw').append(
                    $('<div class="lb">').append( // layer button
                        ...
                        loadIcon = $('<i class="play icon"></i>').toggle(
                          function () { updateLayer(layerId); loadIcon.removeClass('play').addClass('check active'); if (details.is(':visible')) {$('.' + l.I + '-picker').show(); } if (!label.hasClass('active')) label.addClass('active'); },
                          function () { disableLayer(l); loadIcon.removeClass('check active').addClass('play'); $('.' + l.I + '-picker').hide(); if (label.hasClass('active')) label.removeClass('active'); }
                        ), 
...
if (allLayers.length > 0) {
    ...
    $('<a class="btn btn-sm btn-primary" href="' + baseURL + '" style="display:block"><i class="fa fa-home"></i> SHOW ALL LAYERS</a>').appendTo('#layers');
} else {
    // Load all layers
    initLayers();
