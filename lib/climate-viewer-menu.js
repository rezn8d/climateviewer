...
function enableLayer(b, force) {
  ...
  function fail(kmlData) {
    b.after('<p>ERROR: ' + error + '</p>');
  ...
  if (b.hasClass('proxy')) {
    new Cesium.KmlDataSource.load(kmlUrl, {
        ...
        sourceUri: kmlUrl
    }).then( function(kmlData) { success(kmlData); }, function (error) { fail(); } );
  } else {
      new Cesium.KmlDataSource().load(kmlUrl).then( function(kmlData) { success(kmlData); }, function (error) { fail(); } );
  }
      ...
      if (l.section) {
        ...
        if (present) {
          ...
          if (kmlURL == 'nasa') {
            ...
            lbd.append($('<a id="' + l.id + '" class="btn btn-default btn-sm layer-button" href="' + l.kml + '" data-source="' + l.source + '" data-source-url="' + l.sourceURL + '"><i class="fa fa-map-marker"></i> ' + l.name + '</a>'));
            if (l.zoom) { 
              $('#' + l.id).addClass('zoom');
            }
            if (l.proxy) { 
