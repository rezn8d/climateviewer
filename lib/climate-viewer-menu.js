...
function disableLayer(b) {
  var file = b.prop('id');
  var src = activeLayers[b.prop('href')];
  if (!src) return;
  
  var sourceLabel = $("label[for='" + file + "']");
...
function enableLayer(b, force) {
  ...
  if (b.hasClass('proxy')) {
    ...
    new Cesium.KmlDataSource().load(kmlUrl).then(
      ...
      if (l.section) {
        ...
        if (present) {
          ...
          if (kmlURL == 'nasa') {
            ...
            lbd.append($('<a id="' + l.id + '" class="btn btn-default btn-sm layer-button" href="' + l.kml + '" data-source="' + l.source + '" data-source-url="' + l.sourceURL + '"><i class="fa"></i> ' + l.name + '</a>'));
            if (l.zoom) { 
              $('#' + l.id).addClass('zoom');
            }
            if (l.proxy) { 
