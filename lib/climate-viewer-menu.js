...
function loadSliders(src, layerId, details) {
    ...
    var opacity = NSlider({ onChange: function(newValue) { src.alpha = newValue; } }).css({ borderLeftWidth: '150px' });
...
function createPicker(layerId) {
  ...
  picker.on({
    set: function(thingSet) {
      ...
      updateNasa(layerId, selectedDate);
    }
  });
}

function updateNasa(layerId, selectedDate) {
    var b = $('#' + layerId),
...
function disableLayer(b) {
  ...
  if (b.hasClass('positive')) {
      // Update Globe
      if (b.hasClass("wtms") || b.hasClass("wms")) {
        removeImagery(layerId);
...
// TOGGLE SIDEBAR
$('.layer-control-button').detach().prependTo($('.cesium-viewer-toolbar'));
