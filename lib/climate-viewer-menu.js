...
// Set web root url
var baseURL = 'http://climateviewer.net/';
var activeLayers = { };
var enabledLayers = { };
var disabledLayers = { };
...
function disableLayer(b) {
  var file = b.prop('id');
  var src = activeLayers[b.prop('value')];
  if (!src) return;
...
function loadLayers(includeOnly) {
        ...
        for (var i = 0; i < layers.length; i++) {
          ...
          if (kmlURL == 'nasa') {
            ...
            if (l.proxy) { 
              $('#' + l.layer).addClass('proxy');
            }
            if (l.wms) { 
              $('#' + l.layer).addClass('wms');
            }
            if (l.kml) { 
              $('#' + l.layer).addClass('kml');
            }
