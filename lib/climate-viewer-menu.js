...
function enableLayer(b, force) {
  if (force) {
    ...
    var success = function success(geoData) {
      ...
      $('<li class="list-group-item"><strong>Share:</strong> <a href="' + baseURL + 'index.html?layersOn=' + layerId + '">This Layer</a> &bull; <a href="#shareModal" class="shareToggle">Active Layers</a></li>').appendTo(details);
      shareLink();
    };
...
function shareLink() {
    ...
    $('.shareToggle').attr('href', url);
    $('#shareLink').html('<p>Bookmark or share this URL to save your currently active layers:</p><p class="well"><a class="self" href="' + url + '" title="My custom CV3D map">' + url + '</a></p>');
    $('.shareToggle').click(function (event) { event.preventDefault(); $('#shareModal').modal(); });
  }
...
$('.cesium-viewer').click(function () {
  setTimeout(function () {
    if ($('.cesium-infoBox').hasClass('cesium-infoBox-visible')) {
      var mh = $('.cesium-infoBox-iframe').attr('max-height');
      $('.cesium-infoBox-iframe').attr('height', mh);
      console.log('visible');
    } else {
      console.log('nope');
    }
...
var close = $('.cesium-infoBox-close');

var modalTitle = $('.marker-title');
var modalDescription = $('.marker-details');
var modalCamera = $('.marker-camera');

var title = $('.cesium-infoBox-title').detach();
var description = $('.cesium-infoBox-iframe').detach();
var camera = $('.cesium-infoBox-camera').detach();
var close = $('.cesium-infoBox-close').detach();

modalTitle.append(camera).append(title).append(close);
...
function resize() {
  ...
  var infoBoxMaxHeight = infoFrame.css('max-height'),
      infoBoxHeight = infoFrame.css('height');
  if (infoBoxMaxHeight !== infoBoxHeight) {
    infoFrame.css('height', infoBoxMaxHeight);
    var infoBoxDivs = $('.cesium-infoBox').add($('.cesium-infoBox-iframe'));
    var iframeContents = ($('.cesium-infoBox-iframe').contents().find('.cesium-infoBox-description')).add($('.cesium-infoBox-iframe').contents().find('.cesium-infoBox-description-lighter'));
    infoBoxDivs.css('width', '520px');
    iframeContents.css('width', '500px');
