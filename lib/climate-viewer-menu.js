...
function loading(layerId, done, error) {
    ...
    if (done) {
        if (error) {
            b.removeClass('primary').removeClass('kml').removeClass('geojson').removeClass('wms').removeClass('wmts').addClass('error');
            icon.removeClass('spinner loading').addClass('warning sign');
            b.after($('<div class="ui card layer-details"><div class="content"><div class="header main"><i class="warning sign icon"></i>ERROR</div><div class="ui divided very relaxed list usgs-big-today-list"><div class="item"><i class="info icon"></i><div class="content"><div class="header">Error Details</div>' + error + '</div></div></div></div></div>'));
        } else {
            icon.removeClass('spinner loading').addClass('check');
            b.removeClass('primary').addClass('positive');
        }
    } else {
        icon.removeClass('warning circle').removeClass('idea');
        ...
        new Cesium.KmlDataSource.load(geoDataSrc, {
          ...
          if (markerMod) {
            ...
            for (var i = 0; i < entities.length; i++) {
              ...
              billboard.color = Cesium.Color.ORANGERED;
...
function createMenu(includeOnly) {
  ...
  for (var i = 0; i < layers.length; i++) {
    ...
    if (typeof l == "string") {
      //add header
      if (!includeOnly) {
          item = $('<div class="item layer-wrap"></div>').appendTo(layerAccordion);
          $('<a class="title"><i class="dropdown icon"></i>' + l + '</a>').appendTo(item);
          layerBtnGrp = $('<div class="content"></div>').appendTo(item);
      }
    } else {
      //add layer into 'lbd'
      if (l.section) {
        ...
        if (present) {
          if (!layerBtnGrp) {
              ...
              layerBtnGrp = $('<div class="content active ui divided very relaxed list"></div>').appendTo(item);
          }
          var layerButton = $('<div id="' + l.id + '" class="item layer-button"></div>');
          var layerIcon = $('<i id="' + l.id + '-icon" class="icon"></i>').appendTo(layerButton);
          var layeContent = $('<div class="content">' + l.name + '</div>').appendTo(layerButton);
          // LAYER BUTTON ICONS AND NAME
          if (l.largeLayer) { 
            layerButton.attr( 'title', "WARNING: This is a very large dataset,\n will take a while to display,\n and may slow your processor.\n  This layer is intended for powerful computers,\n use at your own risk.");
            layerIcon.addClass('warning circle');
          } 
          if (l.newLayer) {
            layerButton.attr( 'title', "NEW!");
            layerIcon.addClass('idea');
          } 
          if (l.baseLayer) layerButton.addClass('imagery-layer base-layer');
          if (l.wms) layerButton.addClass('imagery-layer wms');
          if (l.wmts == 'nasa-gibs') layerButton.addClass('imagery-layer nasa-gibs');
          if (l.wmts) layerButton.addClass('imagery-layer wmts');
