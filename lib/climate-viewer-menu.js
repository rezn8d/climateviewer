...
function enableLayer(b, force) {
  ...
  function success(kmlData) {
    ...
    $('#' + layerId + '-opacity').slider().on('slideStop', function(ev){ var opacity = $(this).val(); kmlData.opacity = opacity; console.log($(this).val())  });
  }
  function fail(error) {
  ...
  if (b.hasClass("nasa-layer")) {
    ...
    new Cesium.KmlDataSource.load(kmlUrl, {
        ...
        sourceUri: kmlUrl
    }).then( function(kmlData) { save(kmlData); success(kmlData);console.log('kml proxy ' + layerId + ' loaded'); }, function (error) { fail(error); } );
  } else if (b.hasClass('kml')) {
      new Cesium.KmlDataSource().load(kmlUrl).then( function(kmlData) { save(kmlData); success(kmlData);console.log('kml ' + layerId + ' loaded'); }, function (error) { fail(error); } );
  }
...
function loadNasa(layerId) { 
  ...
  if (!ld.length) {
    ...
    $('#' + layerId + '-opacity').slider({ formater: function(value) { return 'Opacity: '+ value; }}).on('slideStop', function(ev) { var alpha = $(this).val(); src.alpha = alpha; console.log(layerId + ' alpha = ' + alpha);  });
    $('#' + layerId + '-brightness').slider({ formater: function(value) { return 'Brightness: '+ value; }}).on('slideStop', function(ev) { var brightness = $(this).val(); src.brightness = brightness; console.log(layerId + ' brightness = ' + brightness);  });
    $('#' + layerId + '-contrast').slider({ formater: function(value) { return 'Contrast: '+ value; }}).on('slideStop', function(ev) { var contrast = $(this).val(); src.contrast = contrast; console.log(layerId + ' contrast = ' + contrast);  });
