...
function resize() {
    ...
    if (clientWidth > mobile) {
        $('#map').height(clientHeight - 50).width(clientWidth - 50).css('min-height', (clientHeight - 50)).addClass('left-50');
    } else {
        $('#map').height(clientHeight - 50).width(clientWidth).css('min-height', (clientHeight - 50));
    }
    if (clientWidth < tiny) {
...
var proxyURL = '/proxy.php?url=';  // production
...
function loadKml(layerId, geoDataSrc, proxy, zoom) {
    var assetLayerGroup = new L.LayerGroup();
    var proxyKml = (proxyURL + encodeURIComponent(geoDataSrc));
    var loadUrl;
    if (proxy) { 
        loadUrl = proxyKml;
