<?php
    ...
    list($sw_lat, $sw_lng) = explode(",", urldecode($_GET["sw"]));

    // explode northeast corner into two variables
    list($ne_lat, $ne_lng) = explode(",", urldecode($_GET["ne"]));
    ...
    else
    {
        ...
        $rows = query("SELECT * FROM places WHERE ? <= latitude AND latitude <= ? AND (? <= longitude OR longitude <= ?) GROUP BY country_code, place_name, admin_code1 ORDER BY RAND() LIMIT 10", $sw_lat, $ne_lat, $sw_lng, $ne_lng);
    }
